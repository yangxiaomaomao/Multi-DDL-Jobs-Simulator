    ...
    static TypeId tid =
        TypeId("ns3::OnOffApplication")
            ...
            .AddAttribute("Priority",
                          "Priority value to be used in the IP header",
                          UintegerValue(0),
                          MakeUintegerAccessor(&OnOffApplication::m_prio),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("FlowId",
                          "Flow Id",
                          UintegerValue(0),
                          MakeUintegerAccessor(&OnOffApplication::m_flowid),
                          MakeUintegerChecker<uint16_t>());
...
OnOffApplication::SendPacket()
{
    ...
    if ((unsigned)actual == m_pktSize)
    {
        ...
        if (InetSocketAddress::IsMatchingType(m_peer))
        {
            NS_LOG_INFO("Flow[" << m_flowid << "]" << "At time " << Simulator::Now().As(Time::S) << " on-off application sent "
